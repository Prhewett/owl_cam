...
def single_capture(picam2, outdir, scp_config=None):
...
def timelapse_capture(picam2, outdir, interval, count, scp_config=None):
    ensure_outdir(outdir)
    i = 0
    try:
...
def button_capture(picam2, outdir, button_pin, scp_config=None, bouncetime=300):
...
def main():
    ...
    try:
        if args.single:
            single_capture(picam2, args.outdir, scp_config=scp_config)
        elif args.timelapse:
            timelapse_capture(picam2, args.outdir, args.interval, args.count, scp_config=scp_config)
        elif args.button:
            button_capture(picam2, args.outdir, args.button_pin, scp_config=scp_config)
    finally:
        picam2.stop()
